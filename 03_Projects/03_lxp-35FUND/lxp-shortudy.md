...
그러나 가장 큰 도전은 기술이 아니라 개념이었다. 프론트엔드와 백엔드, 서로 다른 관점을 가진 팀원들이 LXP의 본질을 어디에 둘 것인가를 다시 정의해야 했다.

서로 다른 배경과 관점을 가진 팀원들이 하나의 비전을 만들어가는 과정. <br>
이것이 우리가 3차 프로젝트에서 마주한 진짜 과제였다.
...
- 반응형 디자인(모바일/데스크톱)
  ...
  모바일에서는 세로 몰입형 경험을 데스크톱에서는 탐색과 시청이 균형 잡힌 UI를 목표로 레이아웃을 분기했다.

- API 호출 최적화: 개별 호출에서 리스트 기반으로
...
- 기술적 성장 : Next.js 서버/클라이언트 컴포넌트 이해, 타입 안정성의 가치
