...
1차, 2차 프로젝트에서 우리가 이해한 LXP(Learning Experience Platform)는 비교적 명확했다.
학습자의 경험을 중심으로 학습 여정을 설계하고, 콘텐츠와 인터랙션을 통해 성장을 돕는 플랫폼.
하지만 3차 프로젝트는 전혀 다른 질문을 던졌다.

“우리는 정말 같은 LXP를 만들고 있는가?”

3차 프로젝트부터는 백엔드와의 본격적인 협업이 시작되었고 기존 React/Redux 기반 코드베이스를 Next.js(App Router)와 TypeScript로 마이그레이션하며 콘텐츠 관리와 서버 통신까지 고려해야 했다.

그러나 가장 큰 도전은 기술이 아니라 개념이었다. 프론트엔드와 백엔드, 서로 다른 관점을 가진 팀원들이 LXP의 본질을 어디에 둘 것인가를 다시 정의해야 했다. <br>
<b>서로 다른 배경과 관점을 가진 팀원들이 하나의 비전을 만들어가는 과정. <br>
이것이 우리가 3차 프로젝트에서 마주한 진짜 과제</br>였다.

이 글은 <u><b>LXP를 어떻게 재정의</b></u>했고 <u><b>왜 그런 선택을 했는지에 대한 회고</b></u>다.
...
- **병행 개발** : 12/1 ~ 12/5 (수업과 병행)
- **집중 개발** : 12/6 ~ 12/9 (전일 개발)
- **서버 통신** : 12/9
- **태스트** : 12/10 ~ 12/12
- **발표** : 12/15
...
- <b>핵심 철학</b> <br>
  논의를 거쳐 우리가 합의한 LXP의 핵심 철학은 두 가지였다. 학습은 ‘시간을 내서 하는 일’이 아니라, 업무 중 혹은 학습 중 막히는 순간에 바로 이어지는 경험이어야 한다는 데 의견이 모였다.
...
이번 프로젝트에서는 주요 도메인을 먼저 정의한 뒤,
각 도메인별로 담당자를 지정해 기능 설계부터 구현까지 책임지는 방식으로 진행했다.

| 구분         | 담당자          | 주요 도메인 / 기능        | 구현 내용                                                                                                                                                                        |
| ------------ | --------------- | ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Frontend** | **[FE] 이서정** | 숏폼 전용 페이지 (Shorts) | - 숏폼 전용 플레이어 UI 구현<br>- Swipe / Drag 기반 콘텐츠 전환<br>- 키보드·버튼 네비게이션 지원<br>- 모바일/데스크톱 반응형 레이아웃 설계<br>- Framer Motion 기반 인터랙션 구현 |
...
| **Backend**  | [BE]팀          | 인증 / 유저 도메인        | - 회원가입·로그인 API 구현<br>- JWT 기반 인증/인가 처리<br>- USER / ADMIN 역할 분리                                                                                              |
| **Backend**  | [BE]팀          | 숏폼 도메인               | - 숏폼 리스트 조회 API<br>- 페이지네이션 기반 데이터 제공<br>- Swagger(OpenAPI) 문서화                                                                                           |
| **공통**     | 전원            | 공통 UI & 협업            | - Header / Footer 공통 컴포넌트 정의<br>- Git Flow 기반 브랜치 전략 운영<br>- Notion, GitHub Issues를 통한 협업                                                                  |
...
- **GitHub Repository** : https://github.com/35FUND
- **프로젝트 문서**
